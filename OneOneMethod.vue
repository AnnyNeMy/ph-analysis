<template>
  <div class="wrap">
    <div class="one-one-method-config-wrap" v-for="(row, index) in OneOneMethodConfigArr" :key="index">
        <div class="del-config"><span class="del-config-butt" @click="delMethodConfiguration(index)">X</span></div>
  ...
  <div class="wrap-footer">
    ...
    <div class="config-date">
     ...
     <div class="config-quantity">
        <span class="p-float-label">
        <Dropdown id="sampleSize"
          v-model="sampleSizeName"
          :options="sampleSizeNames"
        />
         <label for="sampleSize">Выдать данных</label>
         </span>
      </div>

  <div class="config-quantity">
 <Button label="+" class="p-button-rounded p-button-success" 
  :disabled="OneOneMethodConfigArr?.length < 1"
  @click="addMethodConfiguration($event)" />
 </div>
<!--  <div class="config-quantity">
  <Button label="-"  class="p-button-rounded p-button-danger" />
 </div> -->
      ...
      <Button
        label="Рассчитать"
        class="p-button-info"
        @click="invokeCalculate11(OneOneMethodConfigArr)"
      />
    </div>
<!--     <ul>
    ...
    <div>
    <h6 v-if="!!commonTableColumns?.length">Результаты расчета обобщенные</h6><span v-if="!!commonTableColumns?.length && !commonTable?.length">Данные отсутствуют</span>
      <div v-if="!!commonTable?.length"  class="table-wrap">
        <DataTable :value="commonTable" :scrollable="true" scrollHeight="flex" :rowClass="rowClass" >
          <Column v-for="(colDef, ind) in commonTableColumns" :key="ind" :field="colDef.field" :header="colDef.header" bodyStyle="text-align:left"></Column>
        </DataTable>
    </div>
</div>

  <div>
    <h6  v-if="!!signalsTableColumns?.length">Данные сигналов</h6><span v-if="!!signalsTableColumns?.length && !signalsTable?.length">Данные отсутствуют</span>
      <div v-if="!!signalsTable?.length"  class="table-wrap">
        <DataTable :value="signalsTable" :scrollable="true" scrollHeight="flex" :rowClass="rowClass" >
          <Column v-for="(colDef, ind) in signalsTableColumns" :key="ind" :field="colDef.field" :header="colDef.header" bodyStyle="text-align:left"></Column>
        </DataTable>
    </div>
</div>

  <div>
    <h6  v-if="!!dataTableColumns?.length">Таблица результатов расчета</h6><span v-if="!!dataTableColumns?.length && !dataTable?.length">Данные отсутствуют</span>
      <div v-if="!!dataTable?.length"  class="table-wrap">
        <DataTable :value="dataTable" :scrollable="true" scrollHeight="flex" :rowClass="rowClass" >
          <Column v-for="(colDef, ind) in dataTableColumns" :key="ind" :field="colDef.field" :header="colDef.header" bodyStyle="text-align:left"></Column>
        </DataTable>
    </div>
</div>
...
.wrap-footer .config-date, .config-quantity {
  margin-right: 10px;
 /*  margin-left: 10px; */
}
.config-quantity {
...
class OneOneMethod extends Vue {
   @Inject('InitialDataService') InitialDataService!: InitialDataService; 
  OneOneMethodConfigArr: OneOneMethodConfig[] = [];
  oneOneMethodConfigCollections: OneOneMethodConfigCollections | undefined | null = null;
  oneOneMethodConfig: OneOneMethodConfig | undefined = undefined;
  startDate = new Date();
  endDate = new Date();
  sampleSizeName: string | undefined;
  sampleSizeNames:  string[] | undefined;
  ...
  created() {
    this.InitialDataService.getEnumsSampleSizeNames()
    .subscribe(x => {
    ...
    zip(
      ...
      .subscribe((x) => {
        ...
        let objAss = Object.assign({}, this.oneOneMethodConfig)
        this.OneOneMethodConfigArr.push(objAss); 
  ...
  private getMethodParams(methodName: string): Observable<GetMethodParamsResponse> {
    ...
    public invokeCalculate11(oneOneMethodConfigArr: OneOneMethodConfig[]) {
    let req = new InvokeCalculate11Request();
    req.basic = new CalculateParamsBasic(this.sampleSizeName, SampleSize[this.sampleSizeName as keyof typeof SampleSize], this.startDate, this.endDate);
    let paramsArr = oneOneMethodConfigArr.map((item, index) => new CalculateParams(index, item, true)); 
    req.prms = paramsArr;
    return Axios.post("/InvokeCalculate11", req).pipe(
      switchMap((x) => of(x.data)))
      .subscribe(t => {
        console.log(t);

        this.dataTableColumns = t?.headersStock?.length && GetTableDateHelper.getTableColumns(t?.headersStock);
        this.dataTable = t?.table?.length  && GetTableDateHelper.getTableData(t?.table);
      
        this.commonTableColumns = t?.headersStats?.length && GetTableDateHelper.getTableColumns(t?.headersStats);
        this.commonTable = t?.sumStat  && GetTableDateHelper.getTableData(t?.sumStat);

        this.signalsTableColumns = t?.headersSignals?.length && GetTableDateHelper.getTableColumns(t?.headersSignals);

        this.signalsTable = t?.signals?.length && GetTableDateHelper.getTableData(t?.signals);
        });
  }

  public rowClass(table1Data: any) {
     return table1Data?.codeClassName === 1 ? {buy: 'buy', tableRowClass: 'tableRowClass'}: table1Data?.codeClassName === 2 ? 'sell': 'tableRowClass';
  }
}

export default OneOneMethod;
</script>